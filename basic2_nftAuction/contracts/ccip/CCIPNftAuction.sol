...
contract CCIPNftAuction is NftAuction, CCIPReceiverUpgradable {
    ...
    function depositNft(
        ...
        // Same logic as parent contract
        super.depositNft(_tokenId, _duration, _nftAddress, _startPrice);

        // Broadcast to side chains
        _broadcastAuctionInfo();
    ...
    function _broadcastAuctionInfo() internal {

        CCIPMessageProtocal.CCIPMessage memory message = CCIPMessageProtocal.createAuctionInfoMessage(
            tokenId,
            nftAddress,
            tokenAddress,
            seller,
    ...
    function _broadcast(
        ...
        for (uint64 i = 0; i < supportedChainIds.length; i++) {
            uint64 targetChainId = supportedChainIds[i];
